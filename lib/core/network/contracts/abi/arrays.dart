...
class FixedBytes extends AbiType<Uint8List> {
  ...
  void validate() {
    if (length < 0 || length > 32) {
      throw Exception('Invalid length of bytes: $length');
    }
  }
...
class FunctionType extends FixedBytes {
  // 20 bytes - address, 4 - function name
...
// Solidity T type for array whose length is known
...
class DynamicLengthArray<T> extends BaseArrayType<T> {
  ...
  DecodingResult<List<T>> decode(ByteBuffer buffer, int offset) {
    ...
    final arrayType = FixedLengthArray<T>(type: type, length: lengthResult.bytesRead);
